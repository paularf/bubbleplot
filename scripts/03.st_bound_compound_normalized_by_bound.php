...
$id_bound_count = make_id_bound_counts($file, $carbon = true);
$total_bound_counts = total_bound_counts($id_bound_count);

//print_r($id_bound_count);

$id_bound_compound_counts = make_id_bound_compound_counts($file,  $filter = [40,41,42,43,44], $carbon = true); 


//$id_class_compound_counts = make_id_class_compound_counts($file, ["AR"]);

$total_bound_compound_counts = make_total_class_compound_counts($id_bound_compound_counts);


$top_total_bound_compound_counts = top_total_class_compound_counts($total_bound_compound_counts, $rank = 10);



//print_r($total_bound_compound_counts);

$id_bound_compound_relab = make_id_bound_compound_relab_by_bound_counts($total_bound_counts, $id_bound_compound_counts);
//print_r($total_bound_counts);

$top_id_bound_compound_relab = make_top_id_class_compound_relab($id_bound_compound_relab, $top_total_bound_compound_counts);

//print_r($id_bound_compound_relab);

$id_bound_compound_relab = order_arr_by_compound($top_id_bound_compound_relab); //cambiar este parámetro a relab o counts dependiendo de lo que queramos, puede ir: $id_bound_compound_counts, $id_bound_compound_relab, $id_class_compound_counts
...
$min = get_min($bound_st_compound_relab);
//print_r($min);
...
$st_depth_bound_relab_bubbleplot->bubble_scale = 100;
...
echo '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" height="500" width="15000">';
